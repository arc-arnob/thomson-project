	...
	if(array_key_exists("id" , $_SESSION)) {
		...
		if(array_key_exists('ITEM_CD',$_POST)){
			...
			if($_POST['ITEM_CD']==""){
				...
				phpAlert("Item code can't be kept empty");
			}
			if($_POST['AMOUNT_PURCHASED']==""){
			
				phpAlert("Please specify amount purchased");
			}
			if($_POST['PURCHASE_DATE']==""){
			
				phpAlert("Date can't be kept empty");
			}else{
				...
				$amount_purchased = $_POST['AMOUNT_PURCHASED'];
			
				if(array_key_exists('ITEM_CD',$row)){
					$stock= $stock+$amount_purchased;
				
					$query= "INSERT INTO `purchase` (ITEM_CD,AMOUNT_PURCHASED,PURCHASE_DATE) VALUES('".mysqli_real_escape_string($link,$_POST["ITEM_CD"])."','".$_POST['AMOUNT_PURCHASED']."','".$_POST["PURCHASE_DATE"]."' ) ";
					if(mysqli_query($link,$query)){
					
						$query = "UPDATE `items` SET LAST_QTY_PURCHASE_DT= '".$_POST["PURCHASE_DATE"]."'  WHERE ITEM_CD= '".mysqli_real_escape_string($link,$_POST["ITEM_CD"])."'";
						$result = mysqli_query($link , $query);
				...
				phpAlert("Theres no such item is the inventory");
				}
			}
...
<html>
	...
	<div id="logo">
		<h1>Thompson Press: Purchase A New Item</h1>
	</div>
	<hr>
...
<form method="post" id="main">
  <div class="form-group">
	...
	<div class="form-group">
    <label for="AMOUNT_PURCHASED">Quantity Purchased</label>
    <input type="number" class="form-control" id="AMOUNT_PURCHASED" name="AMOUNT_PURCHASED" aria-describedby="emailHelp" placeholder="Enter the amount purchased" required>
    <small id="help" class="form-text text-muted">Please enter a valid amount.</small>
  </div>
	<div class="form-group">
    <label for="PURCHASE_DATE">Date Of Purchase</label>
    <input type="date" class="form-control" value="<?php echo $today; ?>" id="PURCHASE_DATE"  name="PURCHASE_DATE" placeholder="Enter the date of purchase" required>
  </div>
